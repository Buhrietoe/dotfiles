...
case "$OPT" in
    ...
    "1") nice ffmpeg -i "$FILENAME" -vf vidstabtransform=smoothing=50:input="$TRANSFORM":optzoom=1:zoom=-20:crop=black,unsharp=5:5:1:3:3:1 -c:v h264 -preset veryfast -crf 23 "$@" -y "$FILENAME"-stabilized.mp4;;
